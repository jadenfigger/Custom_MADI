...
| `fisher_phase4.md` | CURRENT | Executed Phase-4 record: the unrealistic-cell-volume pathology on Mayo_Glioma sub-125, hypotheses H1-H4, the Fisher geometry at the acquisition, and the §4.5 reporting rule. | Current record; every number stands as executed. Finds H2: blow-up voxels fit far above their noise floor and diffuse like free water, and 79.9% of them take one library volume (41.79 pL) at the band's lowest-density, lowest-`v_i` corner. H1's ridge signature does not survive a geometric null that was added post-hoc and is logged as such. Records two defects in existing outputs (the Bayes `vi_map` product of means; `madi_output_glioma_v4.0/map` is an edema sub-187 fit), and bit-identical reproduction of the executed v4.0 Bayes run. | 2026-09-10 |
...
| `fitting_methods.md` | CURRENT | Supported MAP, Bayesian, AMICO, and `--fit-s0` method reference. | Current. Amended 2026-09-10: documents the fit-time `--trust-floor` / `--trust-floor-mode` option added for Phase 4 (H3), and gains an amendment log. | 2026-09-10 |
...
| `provenance/figures/fisher_phase4/` | PROVENANCE | Phase-4 figures and the per-cutoff arm table. | Belongs to `fisher_phase4.md`. Retained because the fit arms and voxel maps they derive from are not in the repository. | 2026-09-10 |
